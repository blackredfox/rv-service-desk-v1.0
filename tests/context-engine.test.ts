...
describe("Context Engine — Integration", () => {
  ...
  it("labor confirmation is non-blocking by default", async () => {
    const { 
      createContext, 
      setLaborDraft,
      isLaborBlocking,
    ...
    expect(context.labor.confirmationRequired).toBe(false);
    
    // Should NOT be blocking
    expect(isLaborBlocking(caseId)).toBe(false);
  });

  it("labor confirmation flow accepts override value", async () => {
    const { 
      createContext, 
      setLaborDraft,
      setMode,
    ...
    clearContext(caseId);
    createContext(caseId, "water_pump");
    setMode(caseId, "labor_confirmation");
    setLaborDraft(caseId, 1.5);
    
    // Technician provides override value
    const result = processMessage(caseId, "make it 2 hours");
    
    expect(result.intent.type).toBe("CONFIRMATION");
    if (result.intent.type === "CONFIRMATION") {
      expect(result.intent.value).toBe(2);
    }
    expect(result.context.labor.confirmedHours).toBe(2);
    expect(result.context.labor.mode).toBe("confirmed");
  });
});

...
describe("LLM Runtime Signals — evidence integrity", () => {
  ...
  // ── Blocker-2 regression: split-token hallucination must be rejected ──
  it("REJECTS 'replaced water heater valve' when tokens only appear split across unrelated transcript segments", () => {
    const splitTranscript = [
      "LP valve open when the panel opens",
      "blown fuse on the 12V line",
      "fuse replaced and restored operation",
      "water heater now works normally",
      "labor 0.3 h",
    ];
    // Every individual token ("replaced", "water", "heater", "valve") appears
    // somewhere in the transcript, but no single segment contains all four.
    // A naive "every token anywhere" grounding would false-accept; the
    // sentence-scoped check must reject.
    const verdict = adjudicateEvidenceSummary({
      proposal: {
        evidence_summary: {
          technician_actions: ["replaced water heater valve"],
        },
      },
      latestUserMessage: "START FINAL REPORT",
      technicianMessages: splitTranscript,
      serverState: STATE,
    });
    expect(verdict.accepted).toBe(false);
    expect(verdict.accepted ? "" : verdict.reason).toBe(REJECT.UNGROUNDED_IN_TRANSCRIPT);
  ...
  it("REJECTS split-token hallucination in required_parts", () => {
    const splitTranscript = [
      "water heater pilot issue reported",
      "gas valve inspected and found within spec",
      "replacement fuse installed",
    ];
    const verdict = adjudicateEvidenceSummary({
      proposal: {
        evidence_summary: {
          required_parts: ["water heater gas valve replacement"],
        },
      },
      latestUserMessage: "START FINAL REPORT",
      technicianMessages: splitTranscript,
  ...
  it("still ACCEPTS a phrase whose tokens appear together in a single segment", () => {
    const coherentTranscript = [
      "water heater complaint reported",
      "replaced the gas valve on the water heater",
      "restored operation confirmed",
    ];
    const verdict = adjudicateEvidenceSummary({
      proposal: {
        evidence_summary: {
          technician_actions: ["replaced the gas valve"],
        },
      },
      latestUserMessage: "START FINAL REPORT",
      technicianMessages: coherentTranscript,
      serverState: STATE,
    });
    expect(verdict.accepted).toBe(true);
  });

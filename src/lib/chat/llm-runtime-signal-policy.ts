 ...
 * This is a permissive grounding check — it rejects obvious inventions
 * (e.g., "valve replacement" when transcript only mentions "fuse")
 * but tolerates paraphrase.
 */
export function isGroundedIn(needle: string, haystack: string): boolean {
  const needleTokens = tokenize(needle);
  if (needleTokens.length === 0) return false;
  const hayLower = haystack.toLowerCase();
  // Cheap "every token appears" check.
...
export function adjudicateReportReadiness(
  ...
  // Every piece of evidence MUST be grounded. Rejects invented facts.
...
export function adjudicateStepIssue(
  ...
  // Evidence strings, if provided, must be grounded.
  if (proposal.evidence) {
    const transcript = joinTranscript(input.technicianMessages);
    for (const ev of proposal.evidence) {
      if (!isGroundedIn(ev, transcript)) {
 ...
 * This is the critical preservation check that prevents e.g. a fuse-replacement
 * transcript from being summarized as a valve replacement.
 */
export function adjudicateEvidenceSummary(
  ...
  const check = (value?: string): boolean =>
    !value || isGroundedIn(value, transcript);

  const checkArr = (values?: string[]): boolean => {
    if (!values) return true;
    for (const v of values) {
      if (!isGroundedIn(v, transcript)) return false;
    }
    return true;
  };

...
const ENRICHMENT_DB: Record<string, Record<string, Record<string, EnrichmentEntry[]>>> = {
  Suburban: {
    water_heater: {
      ...
      wh_9: [{ hint: "Suburban uses an electrode-style flame sensor (not a thermocouple). It must be clean and positioned in the flame path. Carbon buildup is the most common cause of flame-out. Clean with fine emery cloth or replace if cracked." }],
  ...
  Atwood: {
    water_heater: {
      ...
      wh_9: [{ hint: "Atwood pilot models use a standard thermocouple. If flame is present but unit shuts down, the thermocouple is likely weak or mispositioned. Clean with emery cloth and ensure the tip sits fully in the pilot flame." }],
